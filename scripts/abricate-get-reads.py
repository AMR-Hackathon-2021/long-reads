...
def get_files(directory):
    """
    Find the .fasta, .bam and .tab files in a directory
    """
    ...
    for f in os.listdir(directory):
        if f.endswith('.fasta'):
            if files[0] is None:
                ...
                raise Exception("Multiple .fasta files found")
        elif f.endswith('.bam'):
            if files[1] is None:
...
if __name__ == '__main__':
    ...
    if args.dir:
        fastq_file, bam_file, amr_file = get_files(args.dir)
    elif args.amr and args.fastq and args.bam:
        amr_file = args.amr
        fastq_file = args.fastq
        bam_file = args.bam
    else:
        sys.exit('ERROR: Please provide --dir or --amr, --fastq and --bam')

    # Load abricate table
    try:
        amr_table = pd.read_csv(amr_file, sep='\t')
    except IOError:
        sys.exit('ERROR: Cannot open %s' % amr_file)
    ...
    for index, row in amr_table.iterrows():
        if args.out_dir:
            outfile = os.path.join(args.out_dir, row['GENE'] + '.fastq')
        ...
        for r in reads:
            r.comment = 'gene={};product={};accession={};coverage={};identity={}'.format(row['GENE'], row['PRODUCT'], row['ACCESSION'], row['%COVERAGE'], row['%IDENTITY'])
